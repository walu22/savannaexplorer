...
export function buildTripPack(trip, options = {}) {
    ...
    const html = `
        <article class="trip-pack-document">
            ...
            <section class="trip-pack-section"><div class="trip-pack-heading"><span>03</span><div><h2>Readiness checklist</h2><p>${readiness.score}% complete when this pack was generated.</p></div></div><div class="trip-pack-checks">${readiness.html}</div></section>
            <section class="trip-pack-section"><div class="trip-pack-heading"><span>04</span><div><h2>Budget & packing snapshot</h2><p>Working totals and preparation progress.</p></div></div><div class="trip-pack-split"><div><h3>Recorded expenses</h3>${expenseHtml(expenses)}</div><div><h3>Packing progress</h3><p class="trip-pack-big-number">${packedCount}</p><p class="trip-pack-caption">item${packedCount === 1 ? '' : 's'} marked packed in My Safari.</p></div></div></section>
            <section class="trip-pack-section"><div class="trip-pack-heading"><span>05</span><div><h2>Emergency numbers</h2><p>Keep these available offline and verify locally on arrival.</p></div></div><div class="trip-pack-emergencies">${emergencyHtml(countries, options.emergencies || [])}</div></section>
            ${includeNotes && trip.notes ? `<section class="trip-pack-section"><div class="trip-pack-heading"><span>06</span><div><h2>Private trip notes</h2><p>Included by your choice—store this file securely.</p></div></div><div class="trip-pack-notes">${escapeHtml(trip.notes).replace(/\r?\n/g, '<br>')}</div></section>` : ''}
