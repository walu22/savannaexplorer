...
function dashboardHtml(report) {
    ...
    const rows = report.records.map(record => `
        <tr data-status="${record.status}" data-category="${record.category}" data-search="${escapeHtml(`${record.subject} ${record.country} ${record.sourceFile}`.toLowerCase())}">
          ...
          <td><code>${escapeHtml(record.sourceFile)}</code>${sourceLinks(record)}</td>
        </tr>`).join('');
...
<body>
  ...
  <main class="wrap">
    <section class="summary" aria-label="Freshness summary">
      ...
      <article class="metric"><span>Source coverage</span><strong>${report.summary.sourceLinked}/${report.summary.total}</strong><small>${report.summary.missingSource} missing HTTPS sources</small></article>
    </section>
    ...
    <p class="footnote"><strong>Policy:</strong> month-only review dates are conservatively treated as the final day of that month. Travel-advisory links are checked every 30 days; visa, border, park-fee and emergency data every 90 days. This first slice is read-only and tracks source coverage; named reviewer, evidence capture, correction workflow and automated link checks come next.</p>
  </main>
  <script>
